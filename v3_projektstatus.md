...
| Smoke test | `python tools/smoke_test.py` — **338 checks**, headless, in `tools/smoke_suite/` since work order 162 (120 check modules, one group per screen plus a shared core; `tools/smoke_test.py` is the runner). **Two tiers since work order 158**: the bare command runs everything (~200 s here, measured 26 September 2026 — the 72 s this line said was before the rendering checks of 172-174); `--fast` runs the commit gate's 328 (~95 s here). `--screen <name>` prints only that screen's sentences and the core's and is NEVER a gate. See "The gate has two tiers" below |
...
### The research panel's look: a frame, drawn boxes, text that fits — 23 September 2026, work order 166 B, C, D, E

**The outer frame** is the Fleets screen's own inner frame — the one
around its scanner map, orange lamps in the corners — cut out of
`screens/fleets/assets/frame.png` at (205, 189, 1537, 950) by
`tools/make_research_frame.py`. DERIVED: the input is committed, the
output is not, the cut is a plain crop, `setup.py` runs it and a check
rebuilds it byte for byte. The nine-slice corner is 140 px, measured —
the largest corner bracket reaches 132 beyond the rail.

`core/researchframe.py` scales the SOURCE once by `0.5 * layout.scale`
and nine-slices the result, so the corners scale with the window
without stretching. Neither nine-slicer the tree had does that:
`NineSlice` keeps corners at the source's pixel size, `FrameRenderer`
scales them per axis. Marked `outer_frame`, DEVIATION.

**The eight inner boxes and the list popup's window are drawn in
code**, one fill and one rounded outline through `Style.draw_plate` —
the helper the Planets screen and the colony list already share, and
out of the same two palette entries. The `inner_panel` ARTWORK is gone
from both screens' `boxes.json`. Marked `inner_boxes_drawn`.

**The text fits because the BOX moved, not the text.** The rows and
...
technology has, which is what the shrink rule must survive — by
measuring the DIFFERENCE between a render with the text and one
without.

**One counter in the tree.** `colony_move_hd.Counter` is gone and its
three callers take `livedrive.SendCounter`. The two were one job in two
classes, so 165 H's repair reached only the younger one. A check reads
the tree by `ast` and asserts there is exactly one class that replaces
the client's three send methods.
...
|---|---|---:|---:|
| **Full** — the default, and the pre-push gate | `python tools/smoke_test.py` | 338 | ~200 s |
| **Fast** — the pre-commit gate | `python tools/smoke_test.py --fast` | 328 | ~95 s |
...
`tools/struct_probe.py` (**478** code, 753 total), `screens/galaxy_map/screen.py` (**394** code, 733 total — down from 456 when work order 169 moved the HUD drawing into `hudview.py`, and the floor went to `floorlift.render_floor` in 170; 173 made the floor the shared background, `core.backgrounds`), `tools/colony_list_preview.py` (**403** code, 772 total), `screens/custom_race/screen.py` (**403** code, 569 total — three more since work orders 169 and 170: the picks and score bar as two HUD panels, and the flag that keeps its frame buttons inside its columns), `tools/colony_move_hd.py` (**370** code, 570 total — fifteen lines shorter since work order 166 part E took its `Counter` out: the tree has one send counter now, `livedrive.SendCounter`), `core/editor/editor.py` (**359** code, 430 total), `screens/galaxy_map/renderer.py` (**336** code, 758 total), `tools/ext_diag.py` (**325** code, 473 total), `main.py` (**326** code, 557 total — over since work order 142 C added the debug input switch; 146 added the F8 surface screenshot, a TOOL for live acceptance on a display that renders but cannot be captured; 170 the two lines that apply the saved HUD frame colour at start; 173 the line that opens the player's mod folder).
...
### Info — BUILT, NOT ACCEPTED — work order 175 D, 26 September 2026

`screens/info/`, orion2re `SCREEN_INFO` (9), `INFO::Info_Screen_`
(info.cpp:510-642). HD STATE: **BUILT, NOT ACCEPTED** — the live part is
parked. Inventory: `doc/briefs/175-progress.md` part D.

**What it draws**, in the HUD style: the stardate and the income /
maintenance chart (new `s_player` fields `total_maintenance`,
`maintenance[6]`, header route + 9 of 14 saves summing exactly — the other
five are pre-first-turn saves), the five tabs, and the pages — Tech Review
(the original's four tables, researched applications only, the name and the
...
`texts/info/<key>.txt` in the player's mod folder replaces it; the original's
words are read from the player's own files (new `tools/infotext_extract.py`:
HELP.LBX topic lists, RACESTUF trait names, BILLTEX2 group names) and are
never in the tree or the template, which lists their keys and copies only
OrionLayer's own texts. Every text wraps and scrolls (`infobox`).
