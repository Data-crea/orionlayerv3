...
| Smoke test | `python tools/smoke_test.py` — **303 checks**, headless, in `tools/smoke_suite/` since work order 162 (112 check modules, one group per screen plus a shared core; `tools/smoke_test.py` is the runner). **Two tiers since work order 158**: the bare command runs everything (~72 s here); `--fast` runs the commit gate's 296 (~32 s here). `--screen <name>` prints only that screen's sentences and the core's and is NEVER a gate. See "The gate has two tiers" below |
...
|---|---|---:|---:|
| **Full** — the default, and the pre-push gate | `python tools/smoke_test.py` | 303 | ~72 s |
| **Fast** — the pre-commit gate | `python tools/smoke_test.py --fast` | 296 | ~32 s |
