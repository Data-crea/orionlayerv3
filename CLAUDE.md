...
```bash
python tools/smoke_test.py             # everything — 301 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 294, ~32 s
...
301 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 111 check modules, one group
