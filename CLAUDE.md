...
```bash
python tools/smoke_test.py             # everything — 258 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 251, ~32 s
...
258 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 96 check modules, one group
