...
```bash
python tools/smoke_test.py             # everything — 269 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 262, ~32 s
...
269 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 101 check modules, one group
