...
```bash
python tools/smoke_test.py             # everything — 338 checks, ~200 s
python tools/smoke_test.py --fast      # the commit gate's 328, ~95 s
...
for a full one. The ten push-only checks are declared in
...
**What the trade costs:** a fault only those ten can see lands at push
time, not at commit time. See decision 31 and
`doc/briefs/157-suite-profile.md`.

338 checks, headless, no orion2re needed. **The count must not go
...
And races (`screens/races/`, wire id 6, work order 175 C: up to seven
