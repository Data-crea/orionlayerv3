...
```bash
python tools/smoke_test.py             # everything — 263 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 256, ~32 s
...
263 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 98 check modules, one group
