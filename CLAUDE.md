...
```bash
python tools/smoke_test.py             # everything — 300 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 293, ~32 s
...
300 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 110 check modules, one group
