...
```bash
python tools/smoke_test.py             # everything — 305 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 298, ~32 s
...
305 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 112 check modules, one group
