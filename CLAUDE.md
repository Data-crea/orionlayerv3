...
```bash
python tools/smoke_test.py             # everything — 274 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 267, ~32 s
...
274 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 103 check modules, one group
