...
```bash
python tools/smoke_test.py             # everything — 271 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 264, ~32 s
...
271 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 102 check modules, one group
