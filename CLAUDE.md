...
```bash
python tools/smoke_test.py             # everything — 276 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 269, ~32 s
...
276 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 104 check modules, one group
