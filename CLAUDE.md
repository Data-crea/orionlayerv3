...
```bash
python tools/smoke_test.py             # everything — 265 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 258, ~32 s
...
265 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 99 check modules, one group
