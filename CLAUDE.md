...
```bash
python tools/smoke_test.py             # everything — 267 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 260, ~32 s
...
267 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 100 check modules, one group
