...
```bash
python tools/smoke_test.py             # everything — 256 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 249, ~32 s
...
256 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 95 check modules, one group
