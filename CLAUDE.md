...
```bash
python tools/smoke_test.py             # everything — 261 checks, ~72 s
python tools/smoke_test.py --fast      # the commit gate's 254, ~32 s
...
261 checks, headless, no orion2re needed. **The count must not go
...
tools/smoke_suite/      the smoke test's 97 check modules, one group
