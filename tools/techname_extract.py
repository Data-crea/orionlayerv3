...
A SECOND FILE comes out of the same block since 14 September 2026:
`shipparts_<lang>.json` — specials, armour, shields, weapons and hull
classes for the Planets panel's monster values (fundament 64), indexed
the same way (`core/shipparts.py`). No plural forms.

A THIRD since 18 September 2026 (work order 130 D):
`techfields_<lang>.json` — the research FIELD names (strings 1..82) and
APPLICATION names (83..294), which are the first two tables of the same
block, for the research select screen (`core/technames.py`). Three
files and not three key sets in one, for the reason the second one gives
below: a format bump must not blank a column that never read the new
names.
...
def write_ship_parts(strings, lang, lbx_path, out_dir):
    ...
    colony summary's BUILDING column until re-extraction, for names that
    column never reads. Each table's first string is a sum of the game's
    ...
    with open(path, "w", encoding="utf-8") as handle:
        json.dump(dict({
            "_comment": ("GENERATED by tools/techname_extract.py from "
                         f"{os.path.basename(lbx_path)}. Derived from the "
                         "user's own MOO2 installation — do not edit, do "
                         "not ship. Regenerate after changing the game "
                         "language."),
            "_source": os.path.basename(lbx_path),
            "language": lang,
            "entry": LANGUAGES[lang],
            "strings_in_block": len(strings),
            "format": shipparts.FORMAT_VERSION,
        }, **tables), handle, ensure_ascii=False, indent=1)
        handle.write("\n")
    print(f"wrote {path}: " + ", ".join(
        f"{len(v)} {k}" for k, v in tables.items()))
    return path


def write_tech_names(strings, lang, lbx_path, out_dir):
    """The block's FIRST two tables, for the research screen (130 D).

    Fields 1..82 and applications 0..211 — `_technology_fields[0]` is
    the unused row and is not written. A third file rather than new keys
    in either of the others, for the same reason `write_ship_parts`
    gives: a format bump blanks the column that reads that file, and
    neither the colony summary's BUILDING column nor the Planets panel
    reads a research name.

    The hyper-advanced field TITLE is deliberately absent — those eight
    fields are named from ESTRINGS, not from this block
    (`core/technames.py`).
    """
    need = technames.APP_FIRST_STRING + technames.TECH_APP_COUNT
    if len(strings) < need:
        print(f"research names NOT written: the block holds {len(strings)} "
              f"strings and the walk needs {need} to reach the last "
              f"application")
        return None
    fields = {}
    for index in range(1, technames.TECH_FIELD_COUNT):
        text = decode(strings[technames.FIELD_FIRST_STRING + index])
        if text:
            fields[str(index)] = text
    applications = {}
    for index in range(technames.TECH_APP_COUNT):
        text = decode(strings[technames.APP_FIRST_STRING + index])
        if text:
            applications[str(index)] = text
    path = os.path.join(out_dir, os.path.basename(technames.name_file(lang)))
    with open(path, "w", encoding="utf-8") as handle:
        json.dump({
            "_comment": ("GENERATED by tools/techname_extract.py from "
                         f"{os.path.basename(lbx_path)}. Derived from the "
        ...
        handle.write("\n")
    print(f"wrote {path}: {len(fields)} field names, "
          f"{len(applications)} application names")
    return path
...
def main():
    ...
    write_ship_parts(strings, lang, args.lbx, out_dir)
    write_tech_names(strings, lang, args.lbx, out_dir)
